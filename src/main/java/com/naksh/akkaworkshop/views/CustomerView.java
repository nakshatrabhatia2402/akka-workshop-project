package com.naksh.akkaworkshop.views;
...
public class CustomerView extends View {
    ...
    public static class CustomersUpdater extends TableUpdater<customerEntry> {
        ...
        public Effect<customerEntry> onEvent(customerCreated event) {
            return effects()
                    .updateRow(new customerEntry(event.customerId(), event.FirstName(), event.LastName(), false));
        }

        public Effect<customerEntry> onEvent(customerDeleted event) {
            return effects()
                    .updateRow(new customerEntry(event.customerId(), event.FirstName(), event.LastName(), true));
        }
